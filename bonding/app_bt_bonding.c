...
void app_kv_store_init(void)
{
    ...
    {
        WICED_BT_TRACE("successfully initialized QSPI \n");
    }
    else
    {
...
cy_rslt_t app_bt_restore_bond_data(void)
{
    ...
    if (rslt != CY_RSLT_SUCCESS)
    {
        WICED_BT_TRACE("Bond data not present in the flash!\n");
    }
    return rslt;
}
...
cy_rslt_t app_bt_save_device_link_keys(wiced_bt_device_link_keys_t *link_key)
{
    ...
    if (CY_RSLT_SUCCESS != rslt)
    {
        WICED_BT_TRACE("Flash Write Error,Error code: %" PRIu32 "\n", rslt);
    }
    return rslt;
}
...
bool app_bt_find_device_in_flash(uint8_t *bd_addr)
{
    ...
    {
        WICED_BT_TRACE("Found device in the flash!\n");
        return (true);
    }
    return false;
}
...
cy_rslt_t app_bt_read_local_identity_keys(void)
{
    ...
    if (rslt != CY_RSLT_SUCCESS)
    {
        WICED_BT_TRACE("Error Reading Keys! New Keys need to be generated! \n");
    }
    else
    {
        WICED_BT_TRACE("Identity keys are available in the database.\n");
        rslt = mtb_kvstore_read(&kvstore_obj, "local_irk", (uint8_t *)&identity_keys, &data_size);
        WICED_BT_TRACE("Local identity keys read from Flash: \n");
    }
    return rslt;
}
...
cy_rslt_t app_bt_save_local_identity_key(wiced_bt_local_identity_keys_t id_key)
{
    ...
    if (CY_RSLT_SUCCESS == rslt)
    {
        WICED_BT_TRACE("Local identity Keys saved to Flash \n");
    }
    else
    {
        WICED_BT_TRACE("Flash Write Error,Error code: %" PRIu32 "\n", rslt);
